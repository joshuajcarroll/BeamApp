...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  const tokenProvider = useCallback(async () => {
    if (!user?.id) {
      throw new Error("User not authencticated");
    }
        ...
        action={
          <button
            ...
            className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6
                    rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
          >
            Retry
          </button>
        }
